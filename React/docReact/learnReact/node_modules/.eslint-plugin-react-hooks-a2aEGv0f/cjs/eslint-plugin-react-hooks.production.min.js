...
a.parent.key}
function za(a){function c(){return{isUsed:!1,isSatisfiedRecursively:!1,isSubtreeUsed:!1,children:new Map}}function f(k,n){n=n.split(".");n=N(n);var D;try{for(n.s();!(D=n.n()).done;){var G=D.value,H=k.children.get(G);H||(H=c(),k.children.set(G,H));k=H}}catch(J){n.e(J)}finally{n.f()}return k}function q(k,n,D){n=n.split(".");n=N(n);var G;try{for(n.s();!(G=n.n()).done;){var H=k.children.get(G.value);if(!H)break;D(H);k=H}}catch(J){n.e(J)}finally{n.f()}}function v(k,n,D,G){k.children.forEach(function(H,J){var S=
G(J);H.isSatisfiedRecursively?H.isSubtreeUsed&&D.add(S):H.isUsed?n.add(S):v(H,n,D,function(R){return S+"."+R})})}var F=a.dependencies,t=a.declaredDependencies,h=a.stableDependencies,z=a.externalDependencies,K=a.isEffect,x=c();F.forEach(function(k,n){f(x,n).isUsed=!0;q(x,n,function(D){D.isSubtreeUsed=!0})});t.forEach(function(k){f(x,k.key).isSatisfiedRecursively=!0});h.forEach(function(k){f(x,k).isSatisfiedRecursively=!0});a=new Set;var M=new Set;v(x,a,M,function(k){return k});var m=[],p=new Set,w=
...
function Aa(a){var c=a.declaredDependenciesNode,f=a.componentScope,q=a.scope;return a.declaredDependencies.map(function(v){var F=v.key;v=f.variables.find(function(z){return z.name===F});if(null==v)return null;var t=v.defs[0];if(null==t)return null;if("Variable"===t.type&&"VariableDeclarator"===t.node.type&&"Identifier"===t.node.id.type&&null!=t.node.init){var h=V(t.node.init);if(null!=h)return[v,h]}return"FunctionName"===t.type&&"FunctionDeclaration"===t.node.type?[v,"function"]:"ClassName"===t.type&&
"ClassDeclaration"===t.node.type?[v,"class"]:null}).filter(Boolean).map(function(v){var F=v[0];v=v[1];var t=F.defs[0];a:{for(var h=!1,z=0;z<F.references.length;z++){var K=F.references[z];if(K.writeExpr)if(h){F=!0;break a}else{h=!0;continue}for(var x=K.from;x!==q&&null!=x;)x=x.upper;if(x!==q&&!Ba(c,K.identifier)){F=!0;break a}}F=!1}return{construction:t,depType:v,isUsedOutsideOfHook:F}})}
...
function Y(a,c){if("Identifier"===a.type||"JSXIdentifier"===a.type)return a=a.name,c&&c.set(a,!1),a;if("MemberExpression"!==a.type||a.computed){if("OptionalMemberExpression"!==a.type||a.computed){if("ChainExpression"!==a.type||a.computed)throw Error("Unsupported node type: "+a.type);a=a.expression;if("CallExpression"===a.type)throw Error("Unsupported node type: "+a.type);var f=Y(a.object,c),q=Y(a.property,null);f=f+"."+q;Da(a,c,f);return f}f=Y(a.object,c);q=Y(a.property,null);f=f+"."+q;Da(a,c,f);
return f}f=Y(a.object,c);q=Y(a.property,null);f=f+"."+q;Da(a,c,f);return f}function Ea(a){return"MemberExpression"!==a.type||"Identifier"!==a.object.type||"React"!==a.object.name||"Identifier"!==a.property.type||a.computed?a:a.property}
function Ha(a,c){var f=Ea(a);if("Identifier"!==f.type)return-1;switch(f.name){case "useEffect":case "useLayoutEffect":case "useCallback":case "useMemo":return 0;case "useImperativeHandle":return 1;default:if(f===a&&c&&c.additionalHooks){try{var q=Y(f,null)}catch(v){if(/Unsupported node type/.test(v.message))return 0;throw v;}return c.additionalHooks.test(q)?0:-1}return-1}}
function Ia(a,c){for(var f=[a],q=null;f.length;){q=f.shift();if(("Identifier"===q.type||"JSXIdentifier"===q.type)&&q.type===c.type&&q.name===c.name&&q.range[0]===c.range[0]&&q.range[1]===c.range[1])return q;if(Ba(q,c)){a=0;for(var v=Object.entries(q);a<v.length;a++){var F=v[a],t=F[1];"parent"!==F[0]&&(Ja(t)?(t.parent=q,f.push(t)):Array.isArray(t)&&t.forEach(function(h){Ja(h)&&(h.parent=q,f.push(h))}))}}}return null}
...
exports.rules={"rules-of-hooks":{meta:{type:"problem",docs:{description:"enforces the Rules of Hooks",recommended:!0,url:"https://reactjs.org/docs/hooks-rules.html"}},create:function(a){function c(h){h=N(h.references);var z;try{for(h.s();!(z=h.n()).done;){var K=z.value,x=K.identifier.parent;if("VariableDeclarator"===x.type&&x.init&&"CallExpression"===x.init.type&&x.init.callee&&xa(x.init.callee)){var M=N(K.resolved.references),m;try{for(M.s();!(m=M.n()).done;){var p=m.value;p!==K&&F.add(p.identifier)}}catch(w){M.e(w)}finally{M.f()}}}}catch(w){h.e(w)}finally{h.f()}}
var f=null,q=[],v=[],F=new WeakSet,t={};return t.onCodePathSegmentStart=function(h){return v.push(h)},t.onCodePathSegmentEnd=function(){return v.pop()},t.onCodePathStart=function(){return q.push(new Map)},t.onCodePathEnd=function(h,z){function K(l,B){var A=K.cache,y=A.get(l.id);B=new Set(B);if(B.has(l.id)){A=[].concat(B);l=A.slice(A.indexOf(l.id)+1);l=N(l);var E;try{for(l.s();!(E=l.n()).done;)p.add(E.value)}catch(O){l.e(O)}finally{l.f()}return BigInt("0")}B.add(l.id);if(void 0!==y)return y;if(h.thrownSegments.includes(l))y=
...
1}B.set(l.id,A);return A}var m=q.pop();if(0!==m.size){var p=new Set;K.cache=new Map;x.cache=new Map;M.cache=new Map;var w=x(h.initialSegment),k=wa(z),n=va(z),D=k?oa(k)||na(k):qa(z)||ua(z),G=Infinity,H=N(h.finalSegments),J;try{for(H.s();!(J=H.n()).done;){var S=J.value;if(S.reachable){var R=M(S);R<G&&(G=R)}}}catch(l){H.e(l)}finally{H.f()}m=N(m);var P;try{for(m.s();!(P=m.n()).done;){var ca=P.value,T=ca[0],ra=ca[1];if(T.reachable){var ia=0===T.nextSegments.length?G<=M(T):G<M(T),Q=K(T)*x(T),fa=p.has(T.id),
Z=N(ra),aa;try{for(Z.s();!(aa=Z.n()).done;){var I=aa.value;fa&&!ya(I)&&a.report({node:I,message:'React Hook "'+a.getSource(I)+'" may be executed more than once. Possibly because it is called in a loop. React Hooks must be called in the exact same order in every component render.'});if(D){if(!fa&&Q!==w&&!ya(I)){var ba='React Hook "'+a.getSource(I)+'" is called conditionally. React Hooks must be called in the exact same order in every component render.'+(ia?" Did you accidentally call a React Hook after an early return?":
"");a.report({node:I,message:ba})}}else if(z.parent&&("MethodDefinition"===z.parent.type||"ClassProperty"===z.parent.type)&&z.parent.value===z){var L='React Hook "'+a.getSource(I)+'" cannot be called in a class component. React Hooks must be called in a React function component or a custom React Hook function.';a.report({node:I,message:L})}else if(k){var U='React Hook "'+a.getSource(I)+'" is called in function "'+(a.getSource(k)+'" that is neither a React function component nor a custom React Hook function. React component names must start with an uppercase letter. React Hook names must start with the word "use".');
a.report({node:I,message:U})}else if("Program"===z.type){var ja='React Hook "'+a.getSource(I)+'" cannot be called at the top level. React Hooks must be called in a React function component or a custom React Hook function.';a.report({node:I,message:ja})}else if(n&&!ya(I)){var da='React Hook "'+a.getSource(I)+'" cannot be called inside a callback. React Hooks must be called in a React function component or a custom React Hook function.';a.report({node:I,message:da})}}}catch(l){Z.e(l)}finally{Z.f()}}}}catch(l){m.e(l)}finally{m.f()}}},
t.CallExpression=function(h){if(na(h.callee)){var z=q[q.length-1],K=v[v.length-1],x=z.get(K);x||(x=[],z.set(K,x));x.push(h.callee)}"Identifier"===h.callee.type&&("useEffect"===h.callee.name||xa(h.callee))&&0<h.arguments.length&&(f=h)},t.Identifier=function(h){null==f&&F.has(h)&&"CallExpression"!==h.parent.type&&a.report({node:h,message:"`"+a.getSource(h)+'` is a function created with React Hook "useEffectEvent", and can only be called from the same component. They cannot be assigned to variables or passed down.'})},
...
W=Y(u,fa),ea;if(ea=n&&"Identifier"===u.type&&("MemberExpression"===u.parent.type||"OptionalMemberExpression"===u.parent.type)&&!u.parent.computed&&"Identifier"===u.parent.property.type&&"current"===u.parent.property.name){for(var X=g.from,ka=!1;X.block!==m;)"function"===X.type&&(ka=null!=X.block.parent&&"ReturnStatement"===X.block.parent.type),X=X.upper;ea=ka}ea&&ia.set(W,{reference:g,dependencyNode:u});if("TSTypeQuery"!==u.parent.type&&"TSTypeReference"!==u.parent.type){var la=g.resolved.defs[0];
if(null!=la&&(null==la.node||la.node.init!==m.parent)&&"TypeParameter"!==la.type)if(Q.has(W))Q.get(W).references.push(g);else{var Fa=g.resolved,La=T(Fa)||ra(Fa);Q.set(W,{isStable:La,references:[g]})}}}}}catch(sa){d.e(sa)}finally{d.f()}b=N(b.childScopes);var Ga;try{for(b.s();!(Ga=b.n()).done;)D(Ga.value)}catch(sa){b.e(sa)}finally{b.f()}}function G(b){b=b.split(".");for(var d="",e=0;e<b.length;e++){if(0!==e){var g=b.slice(0,e+1).join(".");g=!0===fa.get(g);d+=g?"?.":"."}d+=b[e]}return d}function H(b,
d,e,g){return 0===b.size?null:(1<b.size?"":d+" ")+e+" "+(1<b.size?"dependencies":"dependency")+": "+Ka(Array.from(b).sort().map(function(r){return"'"+G(r)+"'"}))+(". Either "+g+" "+(1<b.size?"them":"it")+" or remove the dependency array.")}n&&m.async&&c({node:m,message:"Effect callbacks are synchronous to prevent race conditions. Put the async function inside:\n\nuseEffect(() => {\n  async function fetchData() {\n    // You can await here\n    const response = await MyAPI.getData(someId);\n    // ...\n  }\n  fetchData();\n}, [someId]); // Or [] if effect doesn't need props or state\n\nLearn more about data fetching with Hooks: https://reactjs.org/link/hooks-data-fetching"});
for(var J=t.acquire(m),S=new Set,R=null,P=J.upper;P;){S.add(P);if("function"===P.type)break;P=P.upper}if(P){R=P;var ca=Array.isArray,T=f(function(b){if(!ca(b.defs))return!1;var d=b.defs[0];if(null==d||"VariableDeclarator"!==d.node.type)return!1;var e=d.node.init;if(null==e)return!1;for(;"TSAsExpression"===e.type;)e=e.expression;var g=d.node.parent;if(null==g&&(Ia(R.block,d.node.id),g=d.node.parent,null==g))return!1;if("const"===g.kind&&"Literal"===e.type&&("string"===typeof e.value||"number"===typeof e.value||
...
e});I.push({key:d,node:b});g||ba.add(d)}});var L=za({dependencies:Q,declaredDependencies:I,stableDependencies:aa,externalDependencies:ba,isEffect:n});P=L.unnecessaryDependencies;var U=L.missingDependencies,ja=L.duplicateDependencies,da=L.suggestedDependencies;if(0===ja.size+U.size+P.size)Aa({declaredDependencies:I,declaredDependenciesNode:p,componentScope:R,scope:J}).forEach(function(b){var d=b.construction,e=b.isUsedOutsideOfHook;b=b.depType;var g="function"===b?"useCallback":"useMemo",r="function"===
...
a.getSource(w)+" has "+(H(U,"a","missing","include")||H(P,"an","unnecessary","exclude")||H(ja,"a","duplicate","omit"))+L,suggest:[{desc:"Update the dependencies array to be: ["+da.map(G).join(", ")+"]",fix:function(b){return b.replaceText(p,"["+da.map(G).join(", ")+"]")}}]})}}else{var O=null;Q.forEach(function(b,d){O||b.references.forEach(function(e){if(!O&&h.has(e.identifier)){for(e=e.from;"function"!==e.type;)e=e.upper;e.block===m&&(O=d)}})});if(O){var ta=za({dependencies:Q,declaredDependencies:[],
...
!1,F={additionalHooks:a.options&&a.options[0]&&a.options[0].additionalHooks?new RegExp(a.options[0].additionalHooks):void 0,enableDangerousAutofixThisMayCauseInfiniteLoops:v},t=a.getSourceCode().scopeManager,h=new WeakMap,z=new WeakSet,K=new WeakMap,x=new WeakMap,M=new WeakSet;return{CallExpression:function(m){var p=Ha(m.callee,F);if(-1!==p){var w=m.arguments[p],k=m.callee,n=Ea(k).name,D=m.arguments[p+1];m=/Effect($|[^a-z])/g.test(n);if(w)if(D||m){switch(w.type){case "FunctionExpression":case "ArrowFunctionExpression":q(w,
D,k,n,m);return;case "Identifier":if(!D||D.elements&&D.elements.some(function(G){return G&&"Identifier"===G.type&&G.name===w.name}))return;p=a.getScope().set.get(w.name);if(null==p||null==p.defs)return;p=p.defs[0];if(!p||!p.node)break;if("Variable"!==p.type&&"FunctionName"!==p.type)break;switch(p.node.type){case "FunctionDeclaration":q(p.node,D,k,n,m);return;case "VariableDeclarator":if(p=p.node.init)switch(p.type){case "ArrowFunctionExpression":case "FunctionExpression":q(p,D,k,n,m);return}}break;
default:c({node:k,message:"React Hook "+n+" received a function whose dependencies are unknown. Pass an inline function instead."});return}c({node:k,message:"React Hook "+n+" has a missing dependency: '"+w.name+"'. Either include it or remove the dependency array.",suggest:[{desc:"Update the dependencies array to be: ["+w.name+"]",fix:function(G){return G.replaceText(D,"["+w.name+"]")}}]})}else"useMemo"!==n&&"useCallback"!==n||c({node:k,message:"React Hook "+n+" does nothing when called with only one argument. Did you forget to pass an array of dependencies?"});
