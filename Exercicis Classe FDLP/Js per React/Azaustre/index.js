...
function Compnent({ initialState, totalCount }) {
...
const codiPostal = usuari && usuari.localitzacio && usuari.localitzacio.ciutat && usuari.localitzacio.ciutat.codiPostal;
...
const etiquetes = posts.reduce((etiquetesTotes, post) => [...etiquetesTotes, ...post.etiquetes], []);
console.log(etiquetes);

const etiquetesNoRepetides = Array.from(new Set(posts.reduce((etiquetesTotes, post) => [...etiquetesTotes, ...post.etiquetes], [])));
console.log(etiquetesNoRepetides);

const etiquetesNoRepetides2 = posts.reduce((etiquetesTotes, post) => [...etiquetesTotes, ...post.etiquetes], []).filter((post, index, self) => index === self.indexOf(post));
console.log(etiquetesNoRepetides2);

const nota4a7 = posts.filter(post => post.valoracio >=4 && post.valoracio <=7);
console.log(nota4a7);

const nota4a7bis = posts.filter(post => post.valoracio >= nota.min && post.valoracio <= nota.max,
);
console.log(nota4a7bis);

const indexmes1 = posts.filter((post, index) => index <2 && post.etiquetes.includes("react"));
console.log(indexmes1);

const anyActual = new Date().getFullYear();
console.log(anyActual);

const any2022 = posts.filter(post => post.any === anyActual);
console.log(any2022);
...
const numbers = random.filter(num => typeof(num) === "number" && Number.isInteger(num));
console.log(numbers);
...
const anyTitol = any2022.map(posts => `${posts.titol}, publicat l'any ${anyActual}`);
