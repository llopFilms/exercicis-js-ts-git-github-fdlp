...
function esborrar() {
  let confirmar = confirm(
    "Segur que voleu esborrar la casa" + bioconstruccio[posicio].nom + "?"
  );
...
function modificar() {
  ...
  imatge = prompt(
    "Nova imatge per " +
      nom +
      "? Sense l'extensió '.jpg'. Ara té l'arxiu " +
      imatge +
      ".jpg"
  );
  imatge += ".jpg";
  ...
  titol = prompt(
    "Nou títol per la imatge " + nom + "?. Ara té el títol " + titol
  );
  descripcio = prompt(
    "Nova descripcio per la imatge " +
      nom +
      '?. Ara té la descripcio "' +
      descripcio +
      '"'
  );
...
function sortir() {
  document.getElementById("nom imatge").innerHTML =
    "<strong>Gràcies per la teva visita</strong>";
