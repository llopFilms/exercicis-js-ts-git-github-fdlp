...
function llistaBanc() {
	...
	for (i = 0; i < arrayClients.length; i++) {
		llista +=
			"<li><b>" + arrayClients[i].nom + " " + arrayClients[i].cognom + ":</b> ";

		for (j = 0; j < arrayClients[i].comptes.length; j++) {
			llista +=
				" #" +
				arrayClients[i].comptes[j].numCompte +
				", " +
				arrayClients[i].comptes[j].saldo +
				"€.";
		}
	}
	llista += "</li></ul></small>";
...
function crearClient() {
	...
	escriuDada(
		"avis",
		"Registre de client a nom de <b>" +
			nom +
			" " +
			cognom +
			"</b> registrat satisfactòriament. Ara podeu crear un <b>número de compte</b>."
	);
...
function eliminarClient() {
	...
	confirm(
		"Esteu segurs que voleu esborrar el client " +
			registrat.nom +
			" " +
			registrat.cognom +
			"?"
	...
	let llista =
		...
		" " +
		registrat.cognom +
		"</b> té comptes amb <b>saldo positiu.</b><ul>";
	while (i < registrat.comptes.length) {
		if (registrat.comptes[i].saldo > 0) {
			llista +=
				"<li>El número de compte <b>#" +
				registrat.comptes[i].numCompte +
				" </b>té un saldo positiu de <b> " +
				registrat.comptes[i].saldo +
				" €.</b></li>";
			condicio = true;
		}
		i++;
	...
	if (condicio == true) {
		...
		escriuDada(
			...
			"El client <b>" +
				...
				" " +
				registrat.cognom +
				"</b> ha estat eliminat satisfactòriament."
		);
		botonsCrearClient();
	}
...
function crearCompte() {
	...
	let indexCompteNou = registrat.comptes.findIndex(
		(element) => element.numCompte === numCompteNou
	);

	escriuDada(
		"avis",
		"Nou compte corrent número <b>#" +
			registrat.comptes[indexCompteNou].numCompte +
			"</b>, creat a nom de <b>" +
			registrat.nom +
			" " +
			registrat.cognom +
			"</b>. Disposa d'un saldo incial de: <b>" +
			registrat.comptes[indexCompteNou].saldo +
			" €."
	);
...
function mostrarComptes() {
	...
	let llistaComptes =
		...
		" " +
		registrat.cognom +
		", </b>disposeu dels següents comptes:<ul>";
	while (i < registrat.comptes.length) {
		llistaComptes +=
			"<li><b>Número de compte i saldo</b>: " + registrat.comptes[i] + "</li>";
		i++;
	}
	llistaComptes += "</ul>";
	console.table(arrayComptes);
...
function eliminarCompte2() {
	...
	let indexNumCompteEliminar = registrat.comptes.findIndex(
		(element) => element.numCompte === numCompteEliminar
	);
	...
	if (indexNumCompteEliminar != -1) {
		if (registrat.comptes[indexNumCompteEliminar].saldo != 0) {
			escriuDada(
				"avis",
				"El saldo dels compte <b>#" +
					numCompteEliminar +
					", NO</b> és <b>0€</b>. Buideu el compte primer."
			);
		} else {
			confirm(
				"Esteu segurs que voleu eliminar el compte #" + numCompteEliminar + " ?"
			);
...
function ingressar2() {
	...
	let indexNumCompteIngres = registrat.comptes.findIndex(
		(element) => element.numCompte === numCompteIngres
	);
	let saldo;
	...
	if (indexNumCompteIngres != -1) {
		...
		escriuDada(
			...
			"La quantitat <b>" +
				quantitat +
				" €</b> s'ha ingressat satisfactòriament al compte <b>#" +
				numCompteIngres +
				"</b>. El saldo disponible és de<b> " +
				saldo +
				"€. </b>"
		);
	} else {
...
function retirar2() {
	...
	let indexNumCompteRetir = registrat.comptes.findIndex(
		(element) => element.numCompte === numCompteRetir
	);
	let saldo;
	...
	if (indexNumCompteRetir != -1) {
		...
		escriuDada(
			...
			"La quantitat <b>" +
				quantitat +
				" €</b> s'ha retirat satisfactòriament del compte <b>#" +
				numCompteRetir +
				"</b>. El saldo disponible és de<b> " +
				saldo +
				"€. </b>"
...
function inici() {
	...
	if (indexClient !== -1) {
		escriuDada(
			...
			"El DNI número <b>" +
				dniInici +
				" </b> està registrat amb les següents dades: <br>" +
				registrat +
				". <br><br><b>Cliqueu a l'opció que desitgeu.<b>"
		);
		...
		escriuDada(
			...
			"El DNI número <b>" +
				dniInici +
				", NO </b> està associat a cap client.<br><br><b>Voleu crear un registre de client?</b>"
		);
		escriuDadaValue("dniCrear", dniInici);
		botonsCrearClient();
	}
